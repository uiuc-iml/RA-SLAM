...
int main(int argc, char* argv[]) {
  ...
  torch::Tensor ret_prob_map = my_engine.infer_one(rgb_img);
  cv::Mat vis_ht = float_tensor_to_uint8_mat(ret_prob_map[0]);
  cv::Mat vis_lt = float_tensor_to_uint8_mat(ret_prob_map[1]);
