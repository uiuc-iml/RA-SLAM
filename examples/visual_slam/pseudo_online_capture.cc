...
class SemanticSLAMLogger : public DataLogger<SemanticSLAMData> {
 public:
  SemanticSLAMLogger(const std::string& logdir) : logdir_(logdir), DataLogger<SemanticSLAMData>() {}
...
void tracking(const std::shared_ptr<openvslam::config>& cfg, const std::string& vocab_file_path,
  ...
  std::thread t([&]() {
    ...
    while (true) {
      ...
      data.id = SLAM.FeedStereoImages(data.zed_img_left, data.zed_img_right, timestamp / 1e6);
