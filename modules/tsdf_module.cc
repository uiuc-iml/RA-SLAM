...
void TSDFSystem::Integrate(const SE3<float>& posecam_T_world, const cv::Mat& img_rgb,
  ...
  inputs_.push(std::make_unique<TSDFSystemInput>(cam_T_posecam_ * posecam_T_world, img_rgb.clone(),
                                                 img_depth.clone(), prob_map.clone()));
}
...
void TSDFSystem::DownloadAllMesh(const std::string& vertices_path, const std::string& indices_path,
  ...
  std::ofstream pout(prob_path, std::ios::out | std::ios::binary);
  ...
  pout.write((char*)vertex_prob_buffer.data(), vertex_prob_buffer.size() * sizeof(float));
...
void TSDFSystem::Run() {
  while (true) {
    ...
    // tsdf integration
    {
      ...
      tsdf_.Integrate(input->img_rgb, input->img_depth, input->prob_map, max_depth_, intrinsics_,
                      input->cam_T_world);
