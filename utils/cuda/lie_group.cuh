...
class SO3 : public Matrix3<T> {
 public:
  ...
  __device__ __host__ inline SO3<T> Inverse() const {
    return SO3<T>(this->m00, this->m10, this->m20, this->m01, this->m11, this->m21, this->m02,
                  this->m12, this->m22);
  }
};
...
class SE3 : public Matrix4<T> {
 public:
  ...
  __device__ __host__ SE3<T>(const SO3<T>& rot, const Vector3<T>& trans)
      : Matrix4<T>(rot.m00, rot.m01, rot.m02, trans.x, rot.m10, rot.m11, rot.m12, trans.y, rot.m20,
                   rot.m21, rot.m22, trans.z, 0, 0, 0, 1) {}

  __device__ __host__ inline SE3<T> Inverse() const {
    const SO3<T> r_inv(this->m00, this->m10, this->m20, this->m01, this->m11, this->m21, this->m02,
                       this->m12, this->m22);
  ...
  __device__ __host__ inline SO3<T> GetR() const {
    return SO3<T>(this->m00, this->m01, this->m02, this->m10, this->m11, this->m12, this->m20,
                  this->m21, this->m22);
  }
  ...
  __device__ __host__ inline Vector3<T> Apply(const Vector3<T>& vec3) const {
    return Vector3<T>(this->m00 * vec3.x + this->m01 * vec3.y + this->m02 * vec3.z + this->m03,
