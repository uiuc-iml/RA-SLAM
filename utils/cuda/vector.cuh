...
class Vector2 {
 ...
 public:
  ...
  __device__ __host__ explicit Vector2<T>(const T& scalar) : Vector2<T>(scalar, scalar) {}

  __device__ __host__ explicit Vector2<T>(const Vector3<T>& vec3) : Vector2<T>(vec3.x, vec3.y) {}
  ...
  __device__ __host__ inline Vector2<T> operator-() const { return Vector2<T>(-x, -y); }
  ...
  __device__ __host__ inline T dot(const Vector2<T>& rhs) const { return x * rhs.x + y * rhs.y; }
...
class Vector3 {
 ...
 public:
  ...
  __device__ __host__ Vector3<T>(const T& x_, const T& y_, const T& z_) : x(x_), y(y_), z(z_) {}

  __device__ __host__ explicit Vector3<T>(const Vector2<T>& vec2) : Vector3<T>(vec2.x, vec2.y, 1) {}

  __device__ __host__ explicit Vector3<T>(const Vector4<T>& vec4)
      : Vector3<T>(vec4.x, vec4.y, vec4.z) {}

  __device__ __host__ explicit Vector3<T>(const T& scalar) : Vector3<T>(scalar, scalar, scalar) {}
  ...
  __device__ __host__ inline Vector3<T> operator-() const { return Vector3<T>(-x, -y, -z); }
...
class Vector4 {
 ...
 public:
  ...
  __device__ __host__ inline Vector4<T> operator-() const { return Vector4<T>(-x, -y, -z, -w); }
  ...
  __device__ __host__ inline Vector4<Tout> cast() const {
    return Vector4<Tout>(static_cast<Tout>(x), static_cast<Tout>(y), static_cast<Tout>(z),
                         static_cast<Tout>(w));
  }
